...
Explorar usa un contenedor propio de hasta `1520px`, sin ensanchar las vistas de creación, edición o detalle. La sección cartográfica reparte de forma fluida mini lista y mapa y comparte su altura en escritorio. Hasta `800px`, un selector local accesible muestra una sola vista Lista/Mapa sin desmontar paneles, perder selección ni disparar refetch; seleccionar una mini ficha abre la vista de mapa. Al hacerse visible, Leaflet ejecuta `invalidateSize` para corregir su viewport. Los límites visuales del thumbnail y del popup pertenecen a clases específicas del mapa global para no afectar `LocationPicker` ni `PublicLocationMap`.

Los markers públicos se agrupan exclusivamente en cliente mediante `react-leaflet-cluster`/`Leaflet.markercluster`. La capa de clustering envuelve los mismos `Marker` construidos desde `publicLocation`, muestra el recuento y aplica la expansión/zoom estándar sin transformar coordenadas, cambiar el DTO ni intervenir en los estados pending/applied del viewport. La mini lista y los popups continúan siendo las vías textuales accesibles; la accesibilidad generada internamente para el icono del cluster depende de Leaflet.markercluster.

`Home` conserva una única copia efímera de la ubicación concedida para Cerca de mí. Además de parametrizar el listado, la entrega a `GlobalMapSection`, que calcula bounds locales aproximados según el radio, los aplica a su query de viewport y pide al mapa un `fitBounds` programático. No se dibuja un marker del usuario, no se persiste el valor y las publicaciones continúan usando exclusivamente `publicLocation`. Tras un pan/zoom manual, «Buscar en esta zona» solo cambia la query del mapa; el listado mantiene Cerca de mí y la UI explicita la divergencia.
